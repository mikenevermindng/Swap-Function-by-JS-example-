...
type Graph = {
  [key: string]: { // key of token A in pair
    pairWith: {
...
function covertToGraph(pairs: Pool[]): Graph {
  let graph: Graph = {};
...
function findPathAndRate(
  ...
  if (neighbor === destination) {
    ...
    for (const nextNeighbor of graph[neighbor].pairWith) {
...
function findAllPaths(
  ...
  const graph = covertToGraph(pools);
  // init output is an empty array
  const output: RouteAndRate[] = [];
  findPathAndRate(graph, fromToken, destination, [], amount, amount, output);
  return output;
}
...
const result = findAllPaths(pools, fromToken, toToken, amount);
